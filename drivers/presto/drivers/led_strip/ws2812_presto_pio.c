 ...
 *
 * Presto-local fork of zephyr/drivers/led_strip/ws2812_rpi_pico_pio.c with two
 * RP2350B fixes (see pimoroni,ws2812-presto-pio.yaml):
 *   1. pio_set_gpio_base(pio, 16) is called in the parent init, before
 *      pio_add_program(), so the PIO can drive a data pin > GPIO 31 (the Presto
 *      LED line is GP33). The HAL rejects a base change once instruction memory
 *      is used, which is why it must precede the program load.
 *   2. The data pin is taken from the `out-pin` property as an absolute SoC GPIO
 *      number, not DT_GPIO_PIN (which yields a gpio0_hi-relative cell, e.g. 1).
 * The PIO program, colour mapping and DMA path are unchanged from upstream.
 */
...
LOG_MODULE_REGISTER(ws2812_presto_pio, CONFIG_LED_STRIP_LOG_LEVEL);
...
#define DT_DRV_COMPAT pimoroni_ws2812_presto_pio
...
static int ws2812_rpi_pico_pio_init(const struct device *dev)
{
	...
	/*
	 * RP2350B: a PIO instance addresses either GPIO 0-31 or 16-47. The data
	 * pin (GP33) is in the upper window, so move the base to 16 BEFORE the
	 * program is loaded — the HAL refuses a base change once instruction
	 * memory is in use. No-op on parts/pins that don't need it (base 0).
	 */
#if PICO_PIO_USE_GPIO_BASE
	{
		int rc = pio_set_gpio_base(pio, config->gpio_base);

		if (rc != PICO_OK) {
			LOG_ERR("%s: pio_set_gpio_base(%u) failed: %d", dev->name,
				config->gpio_base, rc);
			return -EIO;
		}
	}
...
/* Absolute data pin of the (single) child strip — picks the PIO GPIO base. */
#define WS2812_OUT_PIN(inst) DT_PROP(DT_INST_CHILD(inst, ws2812), out_pin)
...
#define WS2812_RPI_PICO_PIO_INIT(inst)                                                             \
	...
	static const struct ws2812_rpi_pico_pio_config rpi_pico_pio_ws2812_##inst##_config = {     \
		...
		.gpio_base = (WS2812_OUT_PIN(inst) >= 32) ? 16 : 0,                                \
	};                                                                                         \
                                                                                                   \
